...
alter table products add column if not exists original_price numeric(14,2) check (original_price is null or original_price >= 0);
alter table products add column if not exists stock integer not null default 10 check (stock >= 0);
...
alter table products add column if not exists status varchar(20) not null default 'active' check (status in ('active', 'inactive'));
